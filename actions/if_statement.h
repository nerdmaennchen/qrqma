...
template <> struct action<grammar::if_control_statement> : pegtl::change_states<Context, Context, Context> {
    ...
    {
        return pegtl::change_states<Context, Context, Context>::match< Rule, A, M, Action, Control >(std::make_index_sequence<3>{}, in, oldC, oldC.childContext(), oldC.childContext() );
    }

    template <typename Input> 
    static void success(const Input &,Context& outerC, Context& ifC, Context& elseC) {
